 ...
 * Classe: IpbxNetwork (Rede da Empresa / IPBX)
...
class IpbxNetwork extends \CommonDBChild
{
    ...
    {
        return _n('Rede', 'Redes', $nb, 'newmanagement');
    }
    ...
    // ------------------------------------------------------------------
    // Renderiza a lista de redes + formulário de adição
    // ------------------------------------------------------------------
    public static function showForIpbx(Ipbx $ipbx): void
    {
        ...
        $rows = iterator_to_array($DB->request([
            ...
            'ORDER' => 'ip_network ASC',
        ]));

        $twig = plugin_newmanagement_getTwig();
        echo $twig->render('ipbx/tab_network.html.twig', [
            'rows'         => $rows,
            'ipbx_id'      => $ipbx_id,
